...
//	FILE* inputFile;
	...
	{
	    s=0;
	    #pragma omp parallel for schedule(static,10) reduction(+:s)
	    for(j=0; j<n; j++)
	        s=s+a[j][i]*a[j][i];
		...
		//#pragma omp parallel for schedule(static,10) private(j, s)
		for(j=i+1; j<n; j++)
		{
		    s=0;
		    #pragma omp parallel for schedule(static,10) reduction(+:s)
		    ...
		    #pragma omp parallel for schedule(static,10) private(k)
		    for(k=0; k<n; k++)
